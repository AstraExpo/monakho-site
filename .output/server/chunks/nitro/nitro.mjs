...
const _KHb47b = eventHandler((event) => {
...
var K$1 = "Invariant failed";
function V(e, t) {
  if (!e) throw new Error(K$1);
}
function v$1(e) {
...
const l = { stringify: (e) => JSON.stringify(e, function(n, r) {
  ...
  if (v$1(e)) return Object.fromEntries(Object.entries(e).map(([n, r]) => [n, l.encode(r)]));
  ...
  return Array.isArray(e) ? e.map((t) => l.decode(t)) : v$1(e) ? Object.fromEntries(Object.entries(e).map(([t, n]) => [t, l.decode(n)])) : e;
} }, g = (e, t, n, r) => ({ key: e, stringifyCondition: t, stringify: (o) => ({ [`$${e}`]: n(o) }), parseCondition: (o) => Object.hasOwn(o, `$${e}`), parse: (o) => r(o[`$${e}`]) }), S = [g("undefined", (e) => e === void 0, () => 0, () => {
...
function Q(e = {}) {
  const t = {};
  return { get(n, r = {}) {
    return t[n] || (t[n] = G({ ...e, ...r })), t[n];
  } };
}
const b = typeof globalThis < "u" ? globalThis : typeof self < "u" ? self : typeof global < "u" ? global : {}, N = "__unctx__", X = b[N] || (b[N] = Q()), Y = (e, t = {}) => X.get(e, t), q$1 = "__unctx_async_handlers__", $ = b[q$1] || (b[q$1] = /* @__PURE__ */ new Set());
function Z(e) {
  let t;
...
const D = E$1(getRequestURL), ne = E$1(getResponseStatus), re$1 = E$1(getRequestWebStream);
function oe() {
  var e;
...
const se$1 = { "app_functions_ts--getCount_createServerFn_handler": { functionName: "getCount_createServerFn_handler", importer: () => import('../build/functions-RHPYwc0W.mjs') }, "app_functions_ts--updateCount_createServerFn_handler": { functionName: "updateCount_createServerFn_handler", importer: () => import('../build/functions-RHPYwc0W.mjs') } }, le$1 = eventHandler(ie$1), _ = se$1;
...
async function ce$1({ request: e, event: t }) {
  ...
  const a = e.method, s = new URL(e.url, "http://localhost:3000"), c = new RegExp(`${ae("/_server")}/([^/?#]+)`), y = s.pathname.match(c), f = y ? y[1] : null, d = Object.fromEntries(s.searchParams.entries()), C = "createServerFn" in d, I = "raw" in d;
  if (typeof f != "string") throw new Error("Invalid server action param for serverFnId: " + f);
  const x = _[f];
  if (!x) throw console.log("serverFnManifest", _), new Error("Server function info not found for " + f);
  let h;
  if (h = await x.importer(), !h) throw console.log("serverFnManifest", _), new Error("Server function module not resolved for " + f);
  const p = h[x.functionName];
  if (!p) throw console.log("serverFnManifest", _), console.log("fnModule", h), new Error(`Server function module export not resolved for serverFn ID: ${f}`);
  const z = ["multipart/form-data", "application/x-www-form-urlencoded"], w = await (async () => {
    try {
      let i = await (async () => {
      ...
      return i.result instanceof Response ? i.result : !C && (i = i.result, i instanceof Response) ? i : L(i) || O$1(i) ? j(i) : new Response(i !== void 0 ? l.stringify(i) : void 0, { status: ne(H()), headers: { "Content-Type": "application/json" } });
    } catch (i) {
      return i instanceof Response ? i : L(i) || O$1(i) ? j(i) : (console.info(), console.info("Server Fn Error!"), console.info(), console.error(i), console.info(), new Response(l.stringify(i), { status: 500, headers: { "Content-Type": "application/json" } }));
    }
  })();
  if (t.node.req.removeListener("close", o), I) return w;
  if (w.headers.get("Content-Type") === "application/json") {
    const m = await w.clone().text();
...
function _t(e) {
  ...
  const t = /* @__PURE__ */ new Set(), n = (s) => {
    ...
    var _a;
    ((_a = e.notifyOnIndexChange) != null ? _a : true) ? n(s) : r = e.getLocation();
  }, a = async ({ task: s, navigateOpts: i, ...c }) => {
    var _a, _b;
    var d, p;
  ...
  return { get location() {
    ...
    return e.getLength();
  }, subscribers: t, subscribe: (s) => (t.add(s), () => {
    t.delete(s);
  }), push: (s, i, c) => {
    const d = r.state[q];
    i = ie(d + 1, i), a({ task: () => {
      e.pushState(s, i), n({ type: "PUSH" });
    }, navigateOpts: c, type: "PUSH", path: s, state: i });
  }, replace: (s, i, c) => {
    const d = r.state[q];
    i = ie(d, i), a({ task: () => {
      e.replaceState(s, i), n({ type: "REPLACE" });
    }, navigateOpts: c, type: "REPLACE", path: s, state: i });
  }, go: (s, i) => {
    a({ task: () => {
      e.go(s), o({ type: "GO", index: s });
    }, navigateOpts: i, type: "GO" });
  }, back: (s) => {
    a({ task: () => {
      var _a;
      e.back((_a = s == null ? void 0 : s.ignoreBlocker) != null ? _a : false), o({ type: "BACK" });
    }, navigateOpts: s, type: "BACK" });
  }, forward: (s) => {
    a({ task: () => {
      var _a;
      e.forward((_a = s == null ? void 0 : s.ignoreBlocker) != null ? _a : false), o({ type: "FORWARD" });
    }, navigateOpts: s, type: "FORWARD" });
  }, canGoBack: () => r.state[q] !== 0, createHref: (s) => e.createHref(s), block: (s) => {
    var _a;
    var i;
    if (!e.setBlockers) return () => {
    };
    const c = (_a = (i = e.getBlockers) == null ? void 0 : i.call(e)) != null ? _a : [];
    return e.setBlockers([...c, s]), () => {
      var _a2;
      var d, p;
      const m = (_a2 = (d = e.getBlockers) == null ? void 0 : d.call(e)) != null ? _a2 : [];
      (p = e.setBlockers) == null || p.call(e, m.filter((f) => f !== s));
    };
  }, flush: () => {
    var s;
...
function xt(e) {
  ...
  return n.status = "pending", n.resolve = (o) => {
    n.status = "resolved", n.value = o, r(o);
  }, n.reject = (o) => {
...
function $e(e, r) {
  ...
  return f.then(() => {
    ...
    if (z && G && z.index < G.index) {
      const y = z.index;
      a = _.slice(y), t.write(_.slice(0, y) + p()), c = "";
      return;
    }
    let L, I = 0;
...
function Ae(e) {
  ...
  const c = () => {
    ...
    return t < e.length;
  }, d = () => (o = e.charAt(t), o !== "=" && o !== ";" && o !== ",");
  for (; t < e.length; ) {
    for (n = t, i = false; c(); ) if (o = e.charAt(t), o === ",") {
      for (a = t, t += 1, c(), s = t; t < e.length && d(); ) t += 1;
      t < e.length && e.charAt(t) === "=" ? (i = true, t = s, r.push(e.slice(n, a)), n = t) : t = a + 1;
    } else t += 1;
...
const v = { stringify: (e) => JSON.stringify(e, function(t, n) {
  const o = this[t], a = te.find((s) => s.stringifyCondition(o));
  return a ? a.stringify(o) : n;
}), parse: (e) => JSON.parse(e, function(t, n) {
  const o = this[t];
  if (O(o)) {
    const a = te.find((s) => s.parseCondition(o));
    if (a) return a.parse(o);
  }
  return n;
}), encode: (e) => {
  if (Array.isArray(e)) return e.map((t) => v.encode(t));
  if (O(e)) return Object.fromEntries(Object.entries(e).map(([t, n]) => [t, v.encode(n)]));
  const r = te.find((t) => t.stringifyCondition(e));
  return r ? r.stringify(e) : e;
}, decode: (e) => {
  if (O(e)) {
    const r = te.find((t) => t.parseCondition(e));
    if (r) return r.parse(e);
  }
  return Array.isArray(e) ? e.map((r) => v.decode(r)) : O(e) ? Object.fromEntries(Object.entries(e).map(([r, t]) => [r, v.decode(t)])) : e;
} }, J = (e, r, t, n) => ({ key: e, stringifyCondition: r, stringify: (o) => ({ [`$${e}`]: t(o) }), parseCondition: (o) => Object.hasOwn(o, `$${e}`), parse: (o) => n(o[`$${e}`]) }), te = [J("undefined", (e) => e === void 0, () => 0, () => {
}), J("date", (e) => e instanceof Date, (e) => e.toISOString(), (e) => new Date(e)), J("error", (e) => e instanceof Error, (e) => ({ ...e, message: e.message, stack: void 0, cause: e.cause }), (e) => Object.assign(new Error(e.message), e)), J("formData", (e) => e instanceof FormData, (e) => {
  const r = {};
...
Bt(() => {
  ...
  return { getItem: async (n) => {
    if (typeof document > "u") {
      const o = r(n.data), a = e(n, o), s = "/home/astra/Documents/astraExpo/monakho-site/.output/public", { promises: i } = await import('node:fs'), d = (await import('node:path')).join(s, a), [p, m] = await i.readFile(d, "utf-8").then((f) => [v.parse(f), null]).catch((f) => [null, f]);
      if (m && m.code !== "ENOENT") throw m;
      return p;
    }
  }, setItem: async (n, o) => {
    const { promises: a } = await import('node:fs'), s = await import('node:path'), i = r(n.data), c = e(n, i), p = s.join("/home/astra/Documents/astraExpo/monakho-site/.output/public", c);
    await a.mkdir(s.dirname(p), { recursive: true }), await a.writeFile(p, v.stringify(o));
  }, fetchItem: async (n) => {
    const o = r(n.data), a = e(n, o);
    let s = t == null ? void 0 : t.get(a);
...
function Nt(e) {
  ...
  try {
    return { context: v.parse(r), data: e };
  } catch {
    return { data: e };
  }
}
...
function Jt(e) {
  return { _types: void 0, options: { validator: e.validator, validateClient: e.validateClient, client: async ({ next: r, sendContext: t, ...n }) => {
    ...
    if (n.type === "static" && typeof document < "u") {
      ...
      `${a.functionId}${JSON.stringify(a.data)}`;
    }
    const s = await ((o = e.extractedFn) == null ? void 0 : o.call(e, a));
    return r(s);
    ...
    r.serverSsr.injectHtml(async () => {
      try {
        ...
        for (; !(p = await d.read()).done; ) if (p.value) {
          const m = `__TSR_SSR__.injectChunk(${B({ matchId: t.id, id: c.id, chunk: p.value }, { isScriptContext: true, wrap: true, json: true })})`;
          r.serverSsr.injectScript(() => m);
        }
        r.serverSsr.injectScript(() => `__TSR_SSR__.closeStream(${B({ matchId: t.id, id: c.id }, { isScriptContext: true, wrap: true, json: true })})`);
      } catch (d) {
...
function tr(e = {}) {
  ...
  return { use: () => {
    ...
    Re.add(d);
    try {
      const p = o ? o.run(s, i) : i();
      return t || (r = void 0), await p;
    } finally {
      Re.delete(d);
    }
  } };
  ...
  if (typeof ee$1.renderToPipeableStream == "function") {
    ...
    try {
      const a = ee$1.renderToPipeableStream(jsx(ge, { router: r }), { ..._e(e.headers.get("User-Agent")) ? { onAllReady() {
        ...
        console.error("Error in renderToPipeableStream:", s, i);
      } });
    } catch (a) {
